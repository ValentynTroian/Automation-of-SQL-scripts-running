...
for %%A in (*.sql) do set /a cnt_all_scripts+=1

set /A cnt=0

:: Main
for %%G in (*.sql) do set /a cnt+=1 && echo !cnt! of %cnt_all_scripts% && echo %%G && echo. >> %file_temp% && echo %%G >> %file_temp% && sqlcmd -p /S XEPUALVIW0027 -m 1 -E -i"%%G" >> %file_temp% && cls
